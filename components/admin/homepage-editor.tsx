...
export function HomepageEditor() {
  ...
  if (state === "loading" && sections.length === 0) {
    return <div className="rounded-none border border-border bg-card p-8 text-sm text-muted-foreground">{message}</div>
  }

  if (state === "error" && sections.length === 0) {
    return <div className="rounded-none border border-destructive/30 bg-destructive/5 p-8"><p className="font-semibold">{message}</p><Button className="mt-4" variant="outline" onClick={() => void loadSections()}><RotateCcw />Try again</Button></div>
  }

  return <div className="grid gap-6 xl:grid-cols-[18rem_minmax(0,1fr)]">
    <aside className="rounded-none border border-border bg-card p-3">
    ...
    {selected && <section className="min-w-0 rounded-none border border-border bg-card p-5 sm:p-7" aria-labelledby="editor-title">
      ...
      <div className="mt-6 flex items-center justify-between rounded-none border border-border bg-muted/40 px-4 py-3"><div className="flex items-center gap-3"><span className={`grid size-8 place-items-center rounded-none ${isVisible ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}>{isVisible ? <Eye className="size-4" /> : <EyeOff className="size-4" />}</span><div><p className="text-sm font-bold">Section visibility</p><p className="text-xs text-muted-foreground">Hidden sections are omitted from the public homepage.</p></div></div><Button type="button" variant={isVisible ? "secondary" : "outline"} onClick={() => setIsVisible((value) => !value)}>{isVisible ? "Visible" : "Hidden"}</Button></div>
