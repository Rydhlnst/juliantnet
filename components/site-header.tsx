...
export function SiteHeader() {
  return <header className="sticky top-0 z-40 border-b border-border/70 bg-background/95 backdrop-blur">
    <div className="mx-auto flex h-18 max-w-7xl items-center justify-between px-5 lg:px-8">
      ...
      <Sheet><SheetTrigger render={<Button variant="outline" size="icon" className="lg:hidden" aria-label="Buka menu" />}><Menu /></SheetTrigger><SheetContent side="right" className="w-[86vw] max-w-sm bg-background"><SheetHeader><SheetTitle>Internet Cepat</SheetTitle><SheetDescription>Koneksi cepat dan stabil untuk rumah serta bisnis.</SheetDescription></SheetHeader><nav className="flex flex-col gap-1 px-4" aria-label="Navigasi mobile">{navigation.map((item) => <Link key={item.href} className="rounded-lg px-3 py-3 font-semibold hover:bg-muted" href={item.href}>{item.label}</Link>)}<Link className="mt-3 rounded-xl bg-primary px-4 py-3 text-center font-bold text-primary-foreground" href="/cek-coverage">Cek Coverage</Link></nav></SheetContent></Sheet>
    </div>
  </header>
}