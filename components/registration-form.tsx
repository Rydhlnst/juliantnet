...
export function RegistrationForm() {
  ...
  if (submitted) return <div className="rounded-2xl bg-primary/10 p-6 text-primary"><h2 className="text-xl font-black">Data awal sudah diterima.</h2><p className="mt-2 text-sm leading-6">Ini masih demonstrasi tanpa penyimpanan data. Hubungkan pengiriman ini ke CRM atau layanan pendaftaran sebelum dipublikasikan.</p></div>
  return <form className="space-y-5" onSubmit={(event) => { event.preventDefault(); void form.handleSubmit() }} noValidate>
    ...
    <Button type="submit" className="h-11 rounded-xl px-5 font-bold">Lanjutkan konfirmasi</Button>
  </form>
}