...
export function PlanCard({ plan, settings }: { plan: PublicPlan; settings: PublicSettings }) {
  ...
  return <div className="relative pt-3">
    {plan.isPopular && <Badge className="absolute top-0 left-5 z-10 -translate-y-1/2 rounded-full bg-accent px-3 py-1 text-accent-foreground">Paling Populer</Badge>}
    <Card className={`h-full gap-5 border bg-card py-6 shadow-none ${plan.isPopular ? "border-primary/70 ring-1 ring-primary/20" : "border-border/75"}`}>
